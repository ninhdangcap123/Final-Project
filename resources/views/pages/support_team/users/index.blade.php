    ...
    <div class="card">
        ...
        <div class="card-body">
            <ul class="nav nav-tabs nav-tabs-highlight">
                <li class="nav-item"><a href="#new-user" class="nav-link active" data-toggle="tab">Create New User</a>
                </li>
                <li class="nav-item dropdown">
                    <a href="#" class="nav-link dropdown-toggle" data-toggle="dropdown">Manage Users</a>
                    <div class="dropdown-menu dropdown-menu-right">
                        @foreach($user_types as $ut)
                            <a href="#ut-{{ \App\Helpers\DisplayMessageHelper::hash($ut->id) }}" class="dropdown-item"
                               data-toggle="tab">{{ $ut->name }}s</a>
                        @endforeach
                    </div>
                </li>
            </ul>
                                ...
                                <div class="col-md-2">
                                    <div class="form-group">
                                        ...
                                        <select required data-placeholder="Select User" class="form-control select"
                                            ...
                                            @foreach($user_types as $ut)
                                                <option
                                                    value="{{ \App\Helpers\DisplayMessageHelper::hash($ut->id) }}">{{ $ut->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="select form-control" id="bg_id" name="bg_id" data-fouc
                                            ...
                                            @foreach($blood_groups as $bg)
                                                <option
                                                    {{ (old('bg_id') == $bg->id ? 'selected' : '') }} value="{{ $bg->id }}">{{ $bg->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                        ...
                                        <div class="list-icons">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-left">
                                                    {{--View Profile--}}
                                                    <a href="{{ route('users.show', \App\Helpers\DisplayMessageHelper::hash($u->id)) }}"
                                                       class="dropdown-item"><i class="icon-eye"></i> View Profile</a>
                                                    {{--Edit--}}
                                                    <a href="{{ route('users.edit', \App\Helpers\DisplayMessageHelper::hash($u->id)) }}"
                                                       class="dropdown-item"><i class="icon-pencil"></i> Edit</a>
                                                    @if(\App\Helpers\GetUserTypeHelper::userIsSuperAdmin())

                                                        <a href="{{ route('users.reset_pass', \App\Helpers\DisplayMessageHelper::hash($u->id)) }}"
                                                           class="dropdown-item"><i class="icon-lock"></i> Reset
                                                                                                           password</a>
                                                        {{--Delete--}}
                                                        <a id="{{ \App\Helpers\DisplayMessageHelper::hash($u->id) }}"
                                                           onclick="confirmDelete(this.id)" href="#"
                                                           class="dropdown-item"><i class="icon-trash"></i> Delete</a>
                                                        <form method="post"
                                                              id="item-delete-{{ \App\Helpers\DisplayMessageHelper::hash($u->id) }}"
                                                              action="{{ route('users.destroy', \App\Helpers\DisplayMessageHelper::hash($u->id)) }}"
