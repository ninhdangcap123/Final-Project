                ...
                <li class="nav-item dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        @foreach($user_types as $ut)
                            <a href="#ut-{{ \App\Helpers\DisplayMessageHelper::hash($ut->id) }}" class="dropdown-item" data-toggle="tab">{{ $ut->name }}s</a>
                        @endforeach
                    </div>
                </li>
            </ul>
                    ...
                    <h6>Personal Data</h6>
                        <fieldset>
                            <div class="row">
                                ...
                                @foreach($user_types as $ut)
                                    <option value="{{ \App\Helpers\DisplayMessageHelper::hash($ut->id) }}">{{ $ut->name }}</option>
                                @endforeach
                                        </select>
                                    </div>
                                </div>
                                        ...
                                        <div class="list-icons">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-left">
                                                    {{--View Profile--}}
                                                    <a href="{{ route('users.show', \App\Helpers\DisplayMessageHelper::hash($u->id)) }}" class="dropdown-item"><i class="icon-eye"></i> View Profile</a>
                                                    {{--Edit--}}
                                                    <a href="{{ route('users.edit', \App\Helpers\DisplayMessageHelper::hash($u->id)) }}" class="dropdown-item"><i class="icon-pencil"></i> Edit</a>
                                                @if(\App\Helpers\GetUserTypeHelper::userIsSuperAdmin())

                                                        <a href="{{ route('users.reset_pass', \App\Helpers\DisplayMessageHelper::hash($u->id)) }}" class="dropdown-item"><i class="icon-lock"></i> Reset password</a>
                                                        {{--Delete--}}
                                                        <a id="{{ \App\Helpers\DisplayMessageHelper::hash($u->id) }}" onclick="confirmDelete(this.id)" href="#" class="dropdown-item"><i class="icon-trash"></i> Delete</a>
                                                        <form method="post" id="item-delete-{{ \App\Helpers\DisplayMessageHelper::hash($u->id) }}" action="{{ route('users.destroy', \App\Helpers\DisplayMessageHelper::hash($u->id)) }}" class="hidden">@csrf @method('delete')</form>
                                                @endif
