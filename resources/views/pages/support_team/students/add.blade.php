        ...
        <form id="ajax-reg" method="post" enctype="multipart/form-data" class="wizard-form steps-validation"
            ...
            <fieldset>
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="my_course_id">Courses: <span class="text-danger">*</span></label>
                            <select onchange="getClassSections(this.value)" data-placeholder="Choose..." required
                                    name="my_course_id" id="my_course_id" class="select-search form-control">
                                <option value=""></option>
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="section_id">Classes: <span class="text-danger">*</span></label>
                            <select data-placeholder="Select Class First" required name="class_id" id="section_id"
                                    class="select-search form-control">
                                <option
                                    {{ (old('class_id')) ? 'selected' : '' }} value="{{ old('class_id') }}">{{ (old('class_id')) ? 'Selected' : '' }}</option>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select data-placeholder="Choose..." name="my_parent_id" id="my_parent_id"
                                ...
                                @foreach($parents as $p)
                                    <option
                                        {{ (old('my_parent_id') == \App\Helpers\DisplayMessageHelper::hash($p->id)) ? 'selected' : '' }} value="{{ \App\Helpers\DisplayMessageHelper::hash($p->id) }}">{{ $p->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-md-3">
                        ...
                        <select data-placeholder="Choose..." name="dorm_id" id="dorm_id"
                            ...
                            @foreach($dorms as $d)
                                <option
                                    {{ (old('dorm_id') == $d->id) ? 'selected' : '' }} value="{{ $d->id }}">{{ $d->name }}</option>
                            @endforeach
                        </select>
