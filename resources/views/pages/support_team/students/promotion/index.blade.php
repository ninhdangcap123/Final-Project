        ...
        <div class="card">
            <div class="card-header header-elements-inline">
                <h5 class="card-title font-weight-bold">Promote Students From <span
                        class="text-teal">{{ $my_courses->where('id', $fc)->first()->name.' '.$classes->where('id', $fs)->first()->name }}</span>
                                                        TO <span
                        class="text-purple">{{ $my_courses->where('id', $tc)->first()->name.' '.$classes->where('id', $ts)->first()->name }}</span>
                </h5>
                {!! \App\Helpers\GetSystemInfoHelper::getPanelOptions() !!}
            </div>
