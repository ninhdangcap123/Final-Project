...
@section('content')
    ...
    <div class="card">
        <div class="card-header bg-white header-elements-inline">
            <h6 id="ajax-title" class="card-title">Please fill The form Below To Edit record
                                                   of {{ $sr->user->name }}</h6>
        ...
        <form method="post" enctype="multipart/form-data" class="wizard-form steps-validation ajax-update"
              ...
              action="{{ route('students.update', \App\Helpers\DisplayMessageHelper::hash($sr->id)) }}" data-fouc>
            @csrf @method('PUT')
            <h6>Personal data</h6>
            <fieldset>
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input value="{{ $sr->user->address }}" class="form-control" placeholder="Address"
                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input value="{{ $sr->user->email  }}" type="email" name="email" class="form-control"
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select class="select form-control" id="gender" name="gender" required data-fouc
                                ...
                                <option value=""></option>
                                <option {{ ($sr->user->gender  == 'Male' ? 'selected' : '') }} value="Male">Male
                                </option>
                                <option {{ ($sr->user->gender  == 'Female' ? 'selected' : '') }} value="Female">Female
                                </option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            <label>Phone:</label>
                            <input value="{{ $sr->user->phone  }}" type="text" name="phone" class="form-control"
                                   placeholder="">
                        </div>
                    </div>
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input value="{{ $sr->user->phone2  }}" type="text" name="phone2" class="form-control"
                                   placeholder="">
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input name="dob" value="{{ $sr->user->dob  }}" type="text" class="form-control date-pick"
                        ...
                        <div class="form-group">
                            ...
                            <select data-placeholder="Choose..." required name="nal_id" id="nal_id"
                                ...
                                @foreach($nationals as $na)
                                    <option
                                        {{  ($sr->user->nal_id  == $na->id ? 'selected' : '') }} value="{{ $na->id }}">{{ $na->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-md-3">
                        ...
                        <select onchange="getLGA(this.value)" required data-placeholder="Choose.."
                            ...
                            @foreach($states as $st)
                                <option
                                    {{ ($sr->user->state_id  == $st->id ? 'selected' : '') }} value="{{ $st->id }}">{{ $st->name }}</option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="select form-control" id="bg_id" name="bg_id" data-fouc
                                ...
                                @foreach(App\Models\BloodGroup::all() as $bg)
                                    <option
                                        {{ ($sr->user->bg_id  == $bg->id ? 'selected' : '') }} value="{{ $bg->id }}">{{ $bg->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select onchange="getClassSections(this.value)" name="my_course_id" required
                                    id="my_course_id" class="form-control select-search"
                                    data-placeholder="Select Class">
                                <option value=""></option>
                                @foreach($my_courses as $c)
                                    <option
                                        {{ $sr->my_course_id == $c->id ? 'selected' : '' }} value="{{ $c->id }}">{{ $c->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select data-placeholder="Choose..." name="my_parent_id" id="my_parent_id"
                                ...
                                @foreach($parents as $p)
                                    <option
                                        {{ (\App\Helpers\DisplayMessageHelper::hash($sr->parent_id) == \App\Helpers\DisplayMessageHelper::hash($p->id)) ? 'selected' : '' }} value="{{ \App\Helpers\DisplayMessageHelper::hash($p->id) }}">{{ $p->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="year_admitted" data-placeholder="Choose..." id="year_admitted"
                                ...
                                @for($y=date('Y', strtotime('- 10 years')); $y<=date('Y'); $y++)
                                    <option
                                        {{ ($sr->year_admitted == $y) ? 'selected' : '' }} value="{{ $y }}">{{ $y }}</option>
                                @endfor
                            </select>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <select data-placeholder="Choose..." name="dorm_id" id="dorm_id"
                            ...
                            @foreach($dorms as $d)
                                <option
                                    {{ ($sr->dorm_id == $d->id) ? 'selected' : '' }} value="{{ $d->id }}">{{ $d->name }}</option>
                            @endforeach
                        </select>
