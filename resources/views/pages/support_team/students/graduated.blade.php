    ...
    <div class="card">
        ...
        <div class="card-body">
            <ul class="nav nav-tabs nav-tabs-highlight">
                <li class="nav-item"><a href="#all-students" class="nav-link active" data-toggle="tab">All Graduated
                                                                                                       Students</a></li>
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-left">
                                                ...
                                                @if(\App\Helpers\CheckUsersHelper::userIsTeamSA())
                                                    ...
                                                    {{--Not Graduated--}}
                                                    <a id="{{ \App\Helpers\DisplayMessageHelper::hash($s->id) }}"
                                                       href="#" onclick="$('form#ng-'+this.id).submit();"
                                                       class="dropdown-item"><i class="icon-stairs-down"></i> Not
                                                                                                              Graduated</a>
                                                    <form method="post"
                                                          id="ng-{{ \App\Helpers\DisplayMessageHelper::hash($s->id) }}"
                                                          action="{{ route('st.not_graduated', \App\Helpers\DisplayMessageHelper::hash($s->id)) }}"
                ...
                @foreach($my_courses as $mc)
                    <div class="tab-pane fade" id="c{{$mc->id}}">
                        <table class="table datatable-button-html5-columns">
                            ...
                            <tbody>
                            @foreach($students->where('my_course_id', $mc->id) as $s)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td><img class="rounded-circle" style="height: 40px; width: 40px;"
                                             src="{{ $s->user->photo }}" alt="photo"></td>
