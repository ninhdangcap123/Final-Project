...
var GreyPalette = function () {
    ...
    var _componentPnotify = function () {
        ...
        $('.pnotify-launch').on('click', function () {
            new PNotify({
                ...
                icon: 'icon-info22',
                addclass: 'bg-grey-400 border-grey'
            });
        });
    };
    ...
    var _componentNoty = function () {
        ...
        $('.noty-launch').on('click', function () {
            new Noty({
                layout: 'topRight',
                theme: ' alert bg-grey text-white p-0',
    ...
    var _componentSwitchery = function () {
        ...
        var init = new Switchery(switchery, {color: '#777'});
    };
    ...
    var _componentUniform = function () {
        ...
        $('.form-input-styled').uniform({
            ...
            selectClass: 'uniform-select bg-grey border-grey',
            fileButtonClass: 'action btn bg-grey'
        });

        // Material theme example
        $('.form-input-styled-material').uniform({
            selectClass: 'uniform-select text-grey'
        });
    };
    ...
    var _componentPopups = function () {
        ...
        $('[data-popup=tooltip-custom]').tooltip({
            template: '<div class="tooltip"><div class="arrow border-grey"></div><div class="tooltip-inner bg-grey"></div></div>'
        });


        // Popover title
        $('[data-popup=popover-custom]').popover({
            template: '<div class="popover border-grey"><div class="arrow"></div><h3 class="popover-header bg-grey"></h3><div class="popover-body"></div></div>'
        });


        // Popover background color
        $('[data-popup=popover-solid]').popover({
            template: '<div class="popover bg-grey border-grey"><div class="arrow"></div><h3 class="popover-header"></h3><div class="popover-body text-white"></div></div>'
        });
    };
