...
var IndigoPalette = function () {
    ...
    var _componentMultiselect = function () {
        ...
        $('.form-control-multiselect-material').multiselect({
            buttonClass: 'btn btn-light text-indigo'
        });
    };
    ...
    var _componentPnotify = function () {
        ...
        $('.pnotify-launch').on('click', function () {
            new PNotify({
                ...
                icon: 'icon-info22',
                addclass: 'bg-indigo-400 border-indigo'
            });
        });
    };
    ...
    var _componentNoty = function () {
        ...
        $('.noty-launch').on('click', function () {
            new Noty({
                layout: 'topRight',
                theme: ' alert bg-indigo text-white p-0',
    ...
    var _componentUniform = function () {
        ...
        $('.form-input-styled').uniform({
            ...
            selectClass: 'uniform-select bg-indigo border-indigo',
            fileButtonClass: 'action btn bg-indigo'
        });

        // Material theme example
        $('.form-input-styled-material').uniform({
            selectClass: 'uniform-select text-indigo'
        });
    };
    ...
    var _componentPopups = function () {
        ...
        $('[data-popup=tooltip-custom]').tooltip({
            template: '<div class="tooltip"><div class="arrow border-indigo"></div><div class="tooltip-inner bg-indigo"></div></div>'
        });


        // Popover title
        $('[data-popup=popover-custom]').popover({
            template: '<div class="popover border-indigo"><div class="arrow"></div><h3 class="popover-header bg-indigo"></h3><div class="popover-body"></div></div>'
        });


        // Popover background color
        $('[data-popup=popover-solid]').popover({
            template: '<div class="popover bg-indigo border-indigo"><div class="arrow"></div><h3 class="popover-header"></h3><div class="popover-body text-white"></div></div>'
        });
    };
